    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_CHECK_CDC_READ:
            ...
            if(appData.isReadComplete == true)
            {
                ...
                //Commented out for CDConPIC assignment
            }
/*
       ...
       /*   // messages come in one char at a time
        ...
        case APP_STATE_CHECK_UART_RECEIVE:
            ...
            }*///Commented out if for CDConPIC assignment

            _CP0_SET_COUNT(0);
            while(_CP0_GET_COUNT()<10000000){}

            writeRequestResult = USB_DEVICE_CDC_Write(0, &appData.writeTransferHandle, rxbuffer, 4, USB_DEVICE_CDC_TRANSFER_FLAGS_DATA_COMPLETE);
            LATBbits.LATB7 = !LATBbits.LATB7;
            if(USB_DEVICE_CDC_RESULT_OK != writeRequestResult) {
                LATBbits.LATB14 = 1;// turn on another led if there is an error
            }
            else {
                LATBbits.LATB14 = 0;// turn off the led if there is not an error
            }
